...
export default class NewsFlashStore {
  ...
  get activeNewsFlash(): INewsFlash | undefined {
    return this.newsFlashCollection.find(
      (item) => item.id === this.activeNewsFlashId
    );
  }
    ...
    if (this.newsFlashActiveFilter in LOCAL_FILTERS) {
      ...
      fetchNews(queryParams).then((data: any) => {
        ...
        if (data) {
          ...
          console.error(
            `filterNewsFlashCollection(filter:${this.newsFlashActiveFilter}) invalid data:`,
            data
          );
        }
      });
  ...
  infiniteFetchLimit(fetchSize: number): void {
    ...
    if (
      this.newsFlashCollection.length >=
