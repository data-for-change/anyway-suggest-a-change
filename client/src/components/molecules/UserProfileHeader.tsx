...
const UserProfileHeader: React.FC<IUserProfileHeader> = ({
  ...
  const defaultFormInput: IFormInput = {
    ...
    firstName,
    lastName,
    workplace: userDetails.data.roles.filter(
      (role) => role !== ROLE_ADMIN_NAME
    )[0], // first role that is not admin
  };
  ...
  const toggleAdminManagementScreen = (isOpen: boolean) =>
    ...
    <>
      {isAdmin && (
        <Box
          ...
          onClick={() => toggleAdminManagementScreen(true)}
        >
          {t("header.management")}
        </Box>
      )}
      ...
      <Box className={classes.welcomeMsg}>
        <Typography.Body2>{`${t("header.User Greeting")} ${
          defaultFormInput.firstName || ""
        }`}</Typography.Body2>
      </Box>
      <Avatar
