...
const MapDialog: FC<IProps> = ({
  ...
  const [searchScreen, setSearchScreen] = useState<"segment" | "cityAndStreet">(
    "segment"
  );
    ...
    <Dialog isShowing={open} onClose={onClose} maxWidth="lg" fullWidth>
      <Box className={classes.wrapper}>
        <Box className={classes.dialogHeader}>
          <Box
            className={
              searchScreen === "segment" ? classes.chosen : classes.notChosen
            }
            onClick={() => setSearchScreen("segment")}
          >
            <Typography.Title1>
              {t("mapDialog.searchSection")}
            </Typography.Title1>
          </Box>
          <Box
            className={
          ...
          >
            <Typography.Title1>
              {t("mapDialog.searchStreetAndCity")}
            </Typography.Title1>
          </Box>
        </Box>
