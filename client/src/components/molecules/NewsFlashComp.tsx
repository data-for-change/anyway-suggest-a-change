...
const NewsFlashComp: FC<IProps> = ({ news }) => {
  ...
  function getVerificationIcon(verificationText: string) {
    if (verificationText === locationQualificationOptions.REJECTED) {
      return (
        <CancelCircleIcon fill={cherryJamColor} className={classes.icon} />
      );
    } else if (
      verificationText === locationQualificationOptions.VERIFIED ||
  ...
  const verificationIcon = getVerificationIcon(
    news.newsflash_location_qualification
  );
  const criticalIcon = news.critical && (
    <CriticalIcon className={classes.icon} />
  );
  const className =
    news.id === newsFlashStore.activeNewsFlashId ? classes.activeNewsFlash : "";
  const date =
    news.date == null
      ? ""
      : dateFormat(new Date(news.date.replace(/-/g, "/")), locale);
  ...
  return (
    <Link
      key={news.id}
      to={`${settingsStore.currentLanguageRouteString}/newsflash/${news.id}`}
    >
      <Box
        border={1}
      ...
      >
        ...
        >
          <p>
            <Typography.Body5>
              {date}, {news.display_source} {verificationIcon}
              {criticalIcon}
            </Typography.Body5>
          </p>
      ...
      <LocationApprove
        ...
        newFlashTitle={date.concat(", ", news.display_source)}
        news={news}
      />
    </Link>
  );
