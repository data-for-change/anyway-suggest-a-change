...
const HeadOnCollisionsComparisonWidget: FC<IProps> = ({
  data,
  segmetText,
  usePercent,
}) => {
  ...
  const roadNumberSegment: string = ` ${t("onUrban.route")} ${
    widgetsStore.newsFlashWidgetsMetaRoadNumber
  }`;
  const descSegment: string =
    roadNumberSegment == null
      ? ""
      : segmetText.substr(roadNumberSegment.length);
  return (
      ...
      <Box className={classes.primaryContent}>
        <Box className={classes.primaryDesc}>
          <Box className={classes.textHighlight}>
            <Typography.Body2>
              {roadNumberSegment == null ? null : roadNumberSegment}
            </Typography.Body2>
          </Box>
