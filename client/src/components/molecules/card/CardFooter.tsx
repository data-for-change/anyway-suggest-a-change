...
const CardFooter: React.FC<IProps> = ({
  dateComment,
  showRange,
  orgIconPath,
}) => {
  ...
  const lastUpdateDate = dateComment.last_update
    ? dateFormat(new Date(dateComment.last_update), locale)
    : null;
  const dateRange = dateComment.date_range
    ? dateComment.date_range.join("-")
    : null;
  ...
  return (
    <div className={classes.main}>
      ...
      <Box className={classes.logoBox}>
        ...
        <Logo
          src={orgIconPath || LamasImage}
          alt={"Lamas"}
          height={orgIconPath ? 24 : 25}
        />
      </Box>
    </div>
  );
