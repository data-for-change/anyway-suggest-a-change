...
const AccidentsOccurred: FC<AProps> = ({
  accidentsCount,
  singleType,
  labels,
}) => {
  ...
  const elements = [
    ...
    singleType && (
      <Box mr={1} key={4}>
        <Typography.Body1>
          {accidentsCount > 1
            ? t(`textView.${singleType}.plural`)
            : t(`textView.${singleType}.singular`)}{" "}
        </Typography.Body1>
      </Box>
    ),
  ];
...
const TextViewHeader: React.FC<IProps> = ({
  data: { items },
  segmentText,
  singleType,
  totalCount,
