...
export type IFormattedWidgetCountBySeverity =
  IWidgetCountBySeverityTextDataBase<ICountBySeverity>;
...
const TextView: FC<IProps> = ({
  ...
  segmentText,
  severityFieldNames: {
    fatal: fatalFieldName,
    severe: severeFieldName,
    light: lightFieldName,
  ...
  const items: IWidgetCountBySeverityTextDataBase["items"] & any =
    data?.items || {};
  ...
  const howManySeverities = [
    !!countBySeverity.fatal,
    !!countBySeverity.severe,
    !!countBySeverity.light,
  ];
  const isSingleType = howManySeverities.filter(Boolean).length === 1;

  const headerClass = classNames(
    classes.headerBase,
    isSingleType ? classes.headerSingleType : classes.headerList
  );
  return (
