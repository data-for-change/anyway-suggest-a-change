...
const WidgetsTemplate: FC = () => {
  ...
  const organizationName = userStore.userOrganizations
    ? userStore.userOrganizations[0]
    : "";
  const organizationData: OrgLogoData | undefined = logosSourceMap.find(
    (p) => p.key === organizationName
  );
  ...
  const NoDataText = (
    <Typography.Body4>{t("widgets.No data")}</Typography.Body4>
  );

  return (
    <Grid.Container>
      {widgetsData && widgetsData.length > 0 ? widgetCards : NoDataText}{" "}
    </Grid.Container>
  );
};
