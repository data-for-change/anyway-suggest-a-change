...
const LocationApprove: FC<IProps> = ({
  isOpen,
  onClose,
  news,
  newFlashTitle,
  ...
  const userInfo =
    ...
    userStore.userInfo.data.firstName
      ? userStore.userInfo.data.firstName.concat(
          userStore.userInfo.data.lastName
        )
      : null;
  ...
  function handleApproveButton() {
    if (shouldApprove && locationChanged) {
      if (newStreetLoc) {
        updateNews(
          news.id,
          locationQualificationOptions.MANUAL,
          newStreetLoc,
          null
        );
      } else {
  ...
  return (
    <DialogWithHeader
      fullWidth={false}
      isShowing={isOpen}
      onClose={onCloseInitValues}
      title={t("LocationApprove.title")}
    >
      <Box className={classes.mainWindow}>
        ...
        >
          ...
          >
            ...
            <Box mb={2} className={classes.block}>
              <SearchCityAndStreet
                onStreetAndCityChoice={onStreetAndCityChoice}
              />
            </Box>
            <Box display="contents">
              <LocationSelect
                onLocationChange={onMapLocationChange}
                initialLocationGetter={newsGetInitialLocation}
              />
            </Box>
          </Box>
          <Box width={300} height={350} mt={2}>
            ...
            <Box mt={2} className={classes.block}>
              ...
              <Box>
                <ApproveLocationRadioButtons
                  handleApproveStatusChange={handleApproveStatusChange}
        ...
        >
          ...
          >
            <Box sx={{ ml: 2 }}>
              <Button.Small onClick={handleCloseButton}>
                {t("LocationApprove.cancelButton")}
              </Button.Small>
            </Box>
            <Box sx={{ ml: 2 }}>
              <Button.Small onClick={handleApproveButton}>
