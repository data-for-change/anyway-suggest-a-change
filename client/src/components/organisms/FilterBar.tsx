...
import { makeStyles, createStyles, Divider, Grid, Box } from "@mui/material";
...
const FilterBar: FC<IProps> = () => {
  ...
  const onFilterChange = useCallback(
    (value: number) => newsFlashStore.changeTimeFilter(value),
    [newsFlashStore]
  );
          ...
          <Grid container spacing={2} alignItems="center">
              ...
              >
                <Grid item>
                  <Typography.Body2 bold>
                    {widgetsStore.newsFlashWidgetsMetaLocation}
                  </Typography.Body2>
                </Grid>
                <Grid item>
                  <Button.Standard onClick={() => setIsDescOpen(!isDescOpen)}>
                    {isDescOpen
                      ? t("filterBar.Hide Details")
                      : t("filterBar.Show Details")}
                  </Button.Standard>
                </Grid>
              </Grid>
            </Grid>
            <Collapse in={isDescOpen} timeout="auto">
              <Grid item lg={12}>
                <Box mb={2} px={3}>
                  <Typography.Body3>
                    {newsFlashStore.activeNewsFlash?.description}
                  </Typography.Body3>
                </Box>
              </Grid>
            </Collapse>
